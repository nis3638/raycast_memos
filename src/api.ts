...
export const sendMemo = (data: PostMemoParams) => {
  const url = getRequestUrl(`/api/v1/memo`);
  console.log(url);
  console.log(data);
  console.log(JSON.stringify(data)); // 应该正常输出
...
export const getTags = () => {
  const url = getRequestUrl(`/api/v1/memo/-/tags`);
...
export const getAllMemos = (currentUserId?: number) => {
  ...
  const url = getRequestUrl(`/api/v1/memo?filter=${filter}&pageSize=20`);
